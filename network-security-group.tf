...
resource "azurerm_network_security_group" "reformscan" {
  ...
  security_rule {
    ...
    source_address_prefix = [
      "${data.azurerm_public_ip.proxy_out_public_ip.ip_address}",
      "${data.azurerm_public_ip_prefix.aks00_public_ip_prefix.ip_prefix}",
      "${data.azurerm_public_ip_prefix.aks01_public_ip_prefix.ip_prefix}"
    ]
    source_port_range          = "*"
    destination_address_prefix = "*"
    destination_port_range     = "443"
...
resource "azurerm_subnet_network_security_group_association" "subnet_nsg_association" {
  ...
  network_security_group_id = "${azurerm_network_security_group.reformscan.id}"
}
