module "no-envelopes-processed-by-blob-router-alert" {
  ...
  alert_name = "No_envelopes_processed_by_blob_router_-_BSP"
...
|   where message startswith "Processing zip file by blob router service"
...
EOF
  ...
  custom_email_subject       = "Bulk Scan - no envelopes processed by blob router service"
